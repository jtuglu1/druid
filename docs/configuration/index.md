...
|`druid.auth.allowUnauthenticatedHttpOptions`|Boolean|If true, skip authentication checks for HTTP OPTIONS requests. This is needed for certain use cases, such as supporting CORS pre-flight requests. Note that disabling authentication checks for OPTIONS requests will allow unauthenticated users to determine what Druid endpoints are valid (by checking if the OPTIONS request returns a 200 instead of 404), so enabling this option may reveal information about server configuration, including information about what extensions are loaded (if those extensions add extensions).|false|no|
|`druid.auth.emitAuthMetrics`|Boolean|If true, emit service metrics for authorization events. When enabled, Druid emits `auth/forbidden` when access is denied and `auth/exception` when an internal authorization error occurs. See [Security metrics](#security) for details.|false|no|
...
|`druid.coordinator.loadqueuepeon.http.confirmMoveBeforeDrop`|When moving a segment between Historical services, whether the source must keep serving the segment until the Coordinator's view of the cluster confirms that the destination has loaded it. The destination acknowledges a load well before Brokers find out about the new replica, so dropping from the source at that point can leave the segment briefly unreachable from every Broker, silently producing partial query results. Waiting for confirmation means a move takes at least one extra Coordinator run to complete and the source holds the segment for that long, which raises load queue occupancy for moves. It is not bounded to one run: the wait lasts until the destination is observed serving the segment, and if `druid.coordinator.segmentPlacement.enabled` is also set, until Brokers have consumed the change as well. A move whose destination never loads is released by `druid.coordinator.load.timeout`. Set to `false` to restore the previous behavior if this throttles balancing.|`true`|
|`druid.coordinator.segmentPlacement.enabled`|Whether the Coordinator publishes an ordered stream of segment placement changes to Brokers, and waits for them to consume it before dropping a moved segment's last replica from its old server. A Broker syncs each data server independently, so without this it can learn that a segment was dropped from one server before it learns of the server that replaced it, and queries touching that segment return incomplete results in the meantime. Requires `druid.broker.segmentPlacement.enabled` on the Brokers; a Coordinator publishing to Brokers that do not consume has no effect.|`false`|
...
|`druid.manager.segments.pollDuration`|Duration (in ISO 8601 format) between successive syncs of the cache with the metadata store. This property is used only when incremental cache usage mode is to `always` or `ifSynced`.|`PT1M` (1 minute)|
...
|Property|Possible Values|Description|Default|
|--------|---------------|-----------|-------|
|`druid.broker.balancer.type`|`random`, `connectionCount`|Determines how the broker balances connections to Historical processes. `random` chooses randomly, `connectionCount` picks the process with the fewest number of active connections to it.|`random`|
...
|`druid.broker.segment.unavailableSegmentPolicy`|String|What the Broker does when a query touches a segment that should be available but has no server to answer for it. Previously such a segment was dropped from the Broker's timeline as soon as its last server went away, so queries silently returned incomplete results. Set to `IGNORE` for that previous behavior, `ALERT` to keep the segment and report it through logs and metrics while still answering queries, or `FAIL` to fail queries that touch it. `FAIL` honors `druid.query.retryPolicy.numTries` and the per-query context flag that allows partial results.|`ALERT`|
|`druid.broker.segment.unavailableRetentionPeriod`|ISO 8601 period|How long the Broker keeps a segment with no server in its timeline while it cannot establish with the Coordinator whether the segment should be available. Bounds how long an unreachable Coordinator can keep segments pinned.|`PT15M`|
|`druid.broker.segment.unavailableCheckPeriod`|ISO 8601 period|How often the Broker re-checks with the Coordinator on the segments it has no server for. Also governs how quickly a segment that has since been marked unused stops being reported. A segment that has just lost its last server is always checked promptly, regardless of this period.|`PT5S`|
|`druid.broker.segmentPlacement.enabled`|Whether the Broker follows the Coordinator's ordered stream of segment placement changes. While the stream is established the Broker takes it as the authority on segment removals instead of its own per-server syncs, which is what stops a moving segment from appearing to vanish. Must only be enabled where the Coordinator has `druid.coordinator.segmentPlacement.enabled` set: following a Coordinator that publishes nothing leaves the Broker never authoritative and behaving exactly as it does today.|`false`|

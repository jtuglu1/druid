...
public class SegmentAvailabilityTracker
{
  ...
  /**
   * How long a newly tracked segment waits before the Coordinator is asked about it. Short enough not to matter to a
   * query, long enough that a burst of removals -- a move, a handoff, a server going away -- becomes one round rather
   * than one request per segment.
   */
  private static final long IMMEDIATE_CHECK_DELAY_MILLIS = 100;
  ...
  /**
   * Whether a debounced immediate check is already pending, so that a burst of newly tracked segments schedules one
   * round rather than one per segment.
   */
  private final AtomicBoolean immediateCheckScheduled = new AtomicBoolean(false);
   ...
   *
   * @return false only when the policy is {@link UnavailableSegmentPolicy#IGNORE}, in which case the caller should
   *         behave exactly as it did before this feature existed and drop the segment from the timeline. Under every
   *         other policy the segment is always tracked and the timeline entry must be kept.
   */
  public boolean track(SegmentId segmentId, ServerSelector selector)
  {
    ...
    // No cap on what may be queued here: pendingChecks is a subset of trackedSegments, so it adds no growth that
    // tracking has not already committed to, and the work per period is bounded by MAX_SEGMENTS_PER_ROUND in
    // selectSegmentsToCheck() regardless of how long the queue is. A cap could only delay answers, never save memory.
  ...
  /**
   * Schedules a check shortly from now, so that the first answer for a newly tracked segment does not wait a whole
   * {@link BrokerSegmentWatcherConfig#getUnavailableCheckPeriod()}. Segments arriving together within the debounce
   * coalesce into one round, which matters because the events that empty selectors arrive in bursts -- a move, a
   * handoff, or a whole server going away at once.
   */
  private void scheduleImmediateCheck()
  {
    final ScheduledExecutorService exec = this.exec;
    if (exec == null || !immediateCheckScheduled.compareAndSet(false, true)) {
      return;
    }
    ...
    try {
      exec.schedule(
          () -> {
            // Cleared before the round runs, so a segment tracked while it is in progress schedules the next one
            // rather than being left to wait for the periodic sweep.
            immediateCheckScheduled.set(false);
            runChecks();
          },
          IMMEDIATE_CHECK_DELAY_MILLIS,
          TimeUnit.MILLISECONDS
      );
    }
  ...
  /**
   * Picks the segments to ask about this round: unresolved ones first, then any resolved segment that has not been
   * re-checked recently, both bounded by {@link #MAX_SEGMENTS_PER_ROUND}.
   * <p>
   * The bound is what keeps a cluster-wide event -- a tier restarting, a datasource marked unused -- from turning into
   * one enormous burst of Coordinator requests. Anything left over stays in {@link #pendingChecks} and is picked up on
   * a later round, so the backlog drains at a fixed rate rather than being dropped or asked about all at once.
   */
  private Set<SegmentId> selectSegmentsToCheck()
  {
    final Set<SegmentId> toCheck = new HashSet<>();
    for (SegmentId pending : pendingChecks) {
  ...
  private void applyStatuses(
      Map<SegmentId, SegmentAvailabilityStatus> statuses,
      Set<SegmentId> notExpectedOut
  )
  {
    statuses.forEach((segmentId, status) -> {
      ...
      if (status.isKnownNotToBeAvailable()) {
        pendingChecks.remove(segmentId);
      ...
      if (status.getReplicationFactor() == null) {
        // Used, but the Coordinator has not evaluated load rules for it yet -- a fresh leader, or a segment published
        // since the last duty run. That is an absence of an answer, not an answer, so leave the segment UNKNOWN and
        // keep asking. Treating it as "should not be available" would evict a live segment on the strength of a
        // Coordinator that had not got round to it, and nothing would put it back until a server re-announced it.
        log.debug("Coordinator has no replication factor for segment[%s] yet. Leaving it unresolved.", segmentId);
        return;
      }
      ...
      if (tracked.selector.getAvailability() != SegmentAvailability.EXPECTED_AVAILABLE) {
        log.warn(
            "Segment[%s] is used and requires [%s] replicas, but no server holds it and none is loading it."
  ...
  /**
   * Drops segments whose availability could never be established, so that a segment the Coordinator can never be
   * asked about -- because it is permanently unreachable, say -- is not tracked forever.
   * <p>
   * Only unresolved segments expire. One the Coordinator has confirmed as {@code EXPECTED_AVAILABLE} is a real
   * outage, and evicting it would drop it from the timeline and return the silent partial results this class exists
   * to prevent -- while also zeroing the metric that reports the outage, at the moment it is still happening. Such a
   * segment stops being tracked when the Coordinator says it is no longer used, not when a timer runs out. This is
   * what {@link BrokerSegmentWatcherConfig#getUnavailableRetentionPeriod()} documents: the period bounds how long the
   * Broker waits <em>while it cannot establish</em> whether the segment should be available.
   */
  private void expireStaleTracking()
  {
    ...
    trackedSegments.forEach((segmentId, tracked) -> {
      if (tracked.trackedSinceMillis < cutoff
          && tracked.selector.getAvailability() != SegmentAvailability.EXPECTED_AVAILABLE) {
    ...
    if (!expired.isEmpty()) {
      log.warn(
          "Dropping [%d] segments whose availability could not be established for over [%s].",

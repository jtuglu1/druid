...
public class ServerHolder implements Comparable<ServerHolder>
{
  ...
  )
  {
    ...
    for (SegmentHolder holder : queueSnapshot.getSegmentsInQueue()) {
      // HttpLoadQueuePeon already collapses a segment's queued and awaiting-confirmation entries to one holder, since
      // keeping both made the reported action depend on set iteration order. This guards the projected counts against
      // any other snapshot source that does not.

 ...
 * ("Don't emit by default."). Production keeps those methods as no-ops; tests that need to assert on any of them
 * can use this implementation (or {@link Factory}) instead. Currently only {@code query/segments/count} is asserted
 * on; the remaining metrics are emitted so future tests can assert on them too.
 */
